...
public class TelstarCommunicator {
	 ...
	 *
	 * Sets systemOut to the socket's output stream Sets systemIn to the
	 * socket's input stream
	 *
	 * @param socket
	 *            - a socket that has been connected to a TELNET server
	 */
	public boolean attachToSocket(Socket socket) {
		try {
			systemOut = new PrintWriter(socket.getOutputStream(), true);
			systemIn = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			return true;
		} catch (IOException e) {
			log.error("Failed to attach to socket", e);
			return false;
		}
	}
	...
	/**
	 * Read from the TELNET server. best used directly after a command.
	 * 
	...
	public String read() {
		...
		try {
			...
			log.error("Cannot read from server", e);
		}
